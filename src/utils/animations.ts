import { Animated, Platform, ScaleTransform, TranslateXTransform, TranslateYTransform, ViewStyle } from 'react-native';
...
export function defaultScrollInterpolator(index: number, carouselProps: CarouselProps<any>) {
    const range = [1, 0, -1];
...
export function defaultAnimatedStyles(index: number, animatedValue: Animated.Value, carouselProps: CarouselProps<any>) {
    let animatedOpacity: Animated.AnimatedProps<ViewStyle> = {};
    let animatedScale: Animated.AnimatedProps<ViewStyle> = {};

    if (carouselProps.inactiveSlideOpacity && carouselProps.inactiveSlideOpacity < 1) {
        animatedOpacity = {
    ...
    if (carouselProps.inactiveSlideScale && carouselProps.inactiveSlideScale < 1) {
        animatedScale = {
            transform: [{
...
export function shiftAnimatedStyles(index: number, animatedValue: Animated.Value, carouselProps: CarouselProps<any>) {
    let animatedOpacity: Animated.AnimatedProps<ViewStyle> = {};
    let animatedScale: Animated.AnimatedProps<ScaleTransform> | undefined;
    let animatedTranslate: Animated.AnimatedProps<TranslateXTransform> | Animated.AnimatedProps<TranslateYTransform> | undefined;
    ...
    if (carouselProps.inactiveSlideShift !== 0) {
        ...
        animatedTranslate = {
            [translateProp]: animatedValue.interpolate({
                inputRange: [0, 1],
                outputRange: [carouselProps.inactiveSlideShift || 0, 0]
            })
        } as Animated.AnimatedProps<TranslateXTransform> | Animated.AnimatedProps<TranslateYTransform>;
    }
    return {
        ...animatedOpacity,
        transform: [
...
export function stackScrollInterpolator(index: number, carouselProps: CarouselProps<any>) {
...
// This means that the item with the higher zIndex (and therefore the tap receiver) remains the one AFTER the currently active item
// The `elevation` property compensates for that only visually, which is not good enough
export function tinderScrollInterpolator(index: number, carouselProps: CarouselProps<any>) {
    const range = IS_ANDROID ?
        [1, 0, -1, -2, -3] :
        [3, 2, 1, 0, -1];
    const inputRange = getInputRangeFromIndexes(range, index, carouselProps);
