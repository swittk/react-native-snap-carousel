...
const AnimatedFlatList = (react_native_1.FlatList ? react_native_1.Animated.createAnimatedComponent(react_native_1.FlatList) : null);
...
class Carousel extends react_1.Component {
    _activeItem;
    _previousActiveItem;
    _previousFirstItem;
    ...
    _scrollStartActive = 0;
    _scrollEndActive = 0;
    _onLayoutInitDone = false;
    static defaultProps = {
    ...
    _setScrollHandler(props) {
        ...
        this._scrollPos = new react_native_1.Animated.Value(0);
        ...
        this._onScrollHandler = react_native_1.Animated.event(argMapping, scrollEventConfig);
    }
    _needsScrollView() {
    ...
    _initPositionsAndInterpolators(props = this.props) {
        ...
        this._getCustomData(props).forEach((itemData, index) => {
            ...
            if (!this._shouldAnimateSlides(props)) {
                animatedValue = new react_native_1.Animated.Value(1);
            }
            else if (this._shouldUseCustomAnimation()) {
                animatedValue = new react_native_1.Animated.Value(_index === this._activeItem ? 1 : 0);
            }
            else {
                let interpolator;
                if (scrollInterpolator) {
                    ...
                    interpolator = (0, animations_1.stackScrollInterpolator)(_index, props);
                }
                else if (this._shouldUseTinderLayout()) {
                    interpolator = (0, animations_1.tinderScrollInterpolator)(_index, props);
                }
                if (!interpolator || !interpolator.inputRange || !interpolator.outputRange) {
                    interpolator = (0, animations_1.defaultScrollInterpolator)(_index, props);
                }
    ...
    _getSlideAnimation(index, toValue) {
        ...
        return react_native_1.Animated.parallel([
            react_native_1.Animated['timing'](animatedValue, { ...animationCommonOptions, easing: react_native_1.Easing.linear }),
            react_native_1.Animated[activeAnimationType](animatedValue, { ...animationCommonOptions })
        ]);
    }
    _playCustomSlideAnimation(current, next) {
        ...
        react_native_1.Animated.parallel(animations, { stopTogether: false }).start();
    }
    ...
    _getSlideInterpolatedStyle(index, animatedValue) {
        ...
        if (slideInterpolatedStyle) {
            ...
            return (0, animations_1.tinderAnimatedStyles)(index, animatedValue, this.props, layoutCardOffset);
        }
        else if (this._shouldUseStackLayout()) {
            return (0, animations_1.stackAnimatedStyles)(index, animatedValue, this.props, layoutCardOffset);
        }
        else if (this._shouldUseShiftLayout()) {
            return (0, animations_1.shiftAnimatedStyles)(index, animatedValue, this.props);
        }
        else {
            return (0, animations_1.defaultAnimatedStyles)(index, animatedValue, this.props);
        }
    }
    _renderItem(arg) {
        ...
        const Component = animate ? react_native_1.Animated.View : react_native_1.View;
        ...
        return (react_1.default.createElement(Component, { style: [mainDimension, slideStyle, animatedStyle], pointerEvents: 'box-none', ...specificProps }, renderItem({ item, index }, parallaxProps)));
    }
    ...
    _getComponentStaticProps() {
        ...
        const { containerCustomStyle, contentContainerCustomStyle, keyExtractor, sliderWidth, sliderHeight, style, vertical } = this.props;
    ...
    render() {
        ...
        return this._needsScrollView() ? (react_1.default.createElement(ScrollViewComponent, { ...props }, this._getCustomData().map((item, index) => {
            return this._renderItem({ item, index });
        }))) : (react_1.default.createElement(AnimatedFlatList, { ...props }));
    }
}
